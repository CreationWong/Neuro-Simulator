...
class LLMClient:
    ...
    async def _ensure_initialized(self):
        ...
        logger.info(
            "First use of built-in agent's LLMClient, performing initialization..."
        )
        ...
        if provider_type == "gemini":
            ...
            raise ValueError(
                f"Unsupported provider type in agent config: {provider_type}"
            )

        self._initialized = True
        logger.info(
            f"Agent LLM client initialized. Provider: {provider_type.upper()}, Model: {self.model_name}"
        )
